...
*Idea of post post is from two blog post. Data is from F-liiga* 
...
As of January 6, 2025, a little more than half of the season has been played in the F-League and the situation regarding the top scorers is as follows:
...
The highest shooting percentage in the F-League at the time of writing is `r leader_raw` who has `r raw`%. Impressive percentages on the list, but how should we react to this when we notice that there are often quite a few shots? Next, let's briefly look at the basis of Bayesian statistics, Bayes' formula.
...
eb1_raw <- round(players_2425_summary %>% arrange(desc(eb_raw)) %>% head(1) %>% select(raw) %>% pull(),2)
eb1_adjusted <- round(players_2425_summary %>% arrange(desc(eb_raw)) %>% head(1) %>% select(raw) %>% pull(),2)

```


The first on the list is `r eb1`, whose uncorrected (gray) shooting percentage is `r eb1_raw` and corrected (black) `r eb1_adjusted`. We notice that for some of the players on the list there is some difference between the corrected and uncorrected.
...
raw1_raw <- round(players_2425_summary %>% arrange(desc(raw)) %>% head(1) %>% select(raw) %>% pull(),2)
raw1_adjusted <- round(players_2425_summary %>% arrange(desc(raw)) %>% head(1) %>% select(raw) %>% pull(),2)
```

The first on the list is of course still (see Table 2) `r raw1`, whose uncorrected (gray) shooting percentage is `r raw1` and corrected (black) `r raw1_adjusted`. We notice that for some of the players on the list the difference between corrected and uncorrected is really big. More supporting material, i.e. shots, is needed for them!
...
Listalla on tuttuja tähtipelaajia, joiden laukaisuosaminen tunnetaan. 
...
```{r, echo = FALSE}
top_10_final_goals <- read.csv("../data/summary_top_10_final_goals.csv")
#
