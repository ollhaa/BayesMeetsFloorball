...
```
*The idea for this blog came from the desire to combine floorball and data analysis and learn a little bit about web scraping. The methods are mainly from the* [Variance Explained blog](http://varianceexplained.org/r/empirical_bayes_baseball/). *I also noticed that the same idea has been used before by* [Ilari Scheinin](https://ilari.scheinin.fi/ranking-nhls-best-shooters/). *My own added value is to apply this to floorball, do explanatory data analysis and predict the final number of goals scored by each player.*

*The data is from [F-liiga webpage](https://fliiga.com/fi/miehet/) and was obtained by web scraping.F-liiga is the highest level of floorball in Finland.*
...
```{r, echo=FALSE}
players_2425_summary <- read_csv("docs/data/players_2425_summary.csv")
...
```{r, echo=FALSE}
summary_top_10_raw <- read.csv("docs/data/summary_top_10_raw.csv")
...
```{r, echo = FALSE}
summary_top_10_eb <- read_csv("docs/data/summary_top_10_eb.csv")
#
...
```{r, echo = FALSE}
summary_top_10_raw <- read_csv("docs/data/summary_top_10_raw.csv")
#
...
```{r, echo = FALSE}
players_veryhigh_eb_veryhigh_spg <- read_csv("docs/data/players_veryhigh_eb_veryhigh_spg.csv")
...
```{r, echo = FALSE}
top_10_final_goals <- read_csv("docs/data/summary_top_10_final_goals.csv")
